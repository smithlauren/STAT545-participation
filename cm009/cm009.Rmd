...
```{r}
lotr
# this dataset has four columns: Film, Race, Gender, Words. It has 18 rows
```
...
```{r}
(wider_lotr <- lotr %>% 
  pivot_wider(id_cols = c(-Race, -Words), # tells R that I want to change these columns
              names_from = Race, 
              values_from = Words))
```
...
(guest_long <- guest %>% 
  pivot_longer(cols      = c(-party, -name), 
               names_to  = c(".value", "event"), #meal to retain column status, event to value in new column
               names_sep = "_"))
```
...
```{r}
email %>% 
  separate_rows(guest, sep = ",") # how does it know to add it to all of the guests?? didn't work the same in the homework 
```
...
tibble(label = "Sample A",
             x = rnorm(n), # creates random values
             y = x +rnorm(n)) # randon value x plus another random values
```
