...
4. white or no fill backgroud
5. points/lines/etc that are large enough to see
...
- <<Weak. The graph doesn't show any types of errors which makes it hard to assess if the differences are actually significant. The data are visually skewed and it is hard to visualize the fact that the averages are not actually that far apart. We can't really assess if they are "more motivated".>>
...
- <<Weak. Y-axis range is way to large - could cut the axis range to 40-80 degrees. To tell if there is an increase, it would be more helpful to have a trendline included in the graph. There also needs to be labels on the x-axis.>>
...
- <<Very weak. Y-axis is flipped, so the graph actually shows that there was an increase in gun deaths. Should have made the color under the line rather than above. >>

- Main takeaway: how you format axes is critically important and how you add color to graphs can make interpretation difficult.
...
1. we didn't end up doing this in class
1. 
1. 
1. 
1. 
...
```{r, plot_ly syntax}
# making the previous graph interactive
p %>%
  ggplotly()
...
# connect plot.ly account

# upload graph to plot.ly
# api_create(p, filename = '___')
```
