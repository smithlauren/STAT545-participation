...
ggplot(mtcars, aes(wt, mpg)) +
  geom_point() +
  labs(x = "Weight (1000's of lbs)", y = "Miles per Gallon") # This graph shoes weight vs miles per gallon. From mtcars dataset
```
...
ggplot(mtcars, aes(qsec, drat)) + 
  ...
  labs(x = "Quarter mile time",
       y = "Rear axle ratio") # this shows qmt vs rear axle ratio, data from mtcars
```
...
ggplot(my_lm_resid, aes(.resid)) +
  geom_freqpoly(binwidth = 0.5) # make sure not to do "bindwidth", makes graph really incorrect
```
...
# what does "se = F" mean... no explanation in class
```
...
ggplot(gapminder_Zimbabwe, aes(year, lifeExp)) + 
  geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(I(x - 1952), degree = 2)) # use poly(I(x-____)) to shift x axis to 1952 instead of 0
```
...
```{r}
lm_linear <- lm(data = gapminder,formula = FILL_THIS_IN) #did not go over this chunk in class
lm_poly <- lm(data = gapminder,formula = FILL_THIS_IN))
```
`anova` lets you compare between different models.
```{r}
anova(lm_linear,lm_poly) #did not go over this
```
