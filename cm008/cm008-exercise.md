...
ggplot(mpg, aes(cty, hwy)) +
  geom_jitter(alpha = 0.5, size = 1) + #jitter adds visual difference to the data, makes it easier to understand, will change every time
...
```r
gapminder %>% 
  filter(continent == "Americas") %>% #have to switch x and y variables compared to above
